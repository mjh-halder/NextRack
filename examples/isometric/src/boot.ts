/*
 * NextRack bootstrap. Extracted from src/index.ts per ADR-0006.
 *
 * The original isometric demo's index.ts (paper/obstacles/example-graph
 * setup) has been entirely superseded by NextRack's own app shell, which
 * lives here. The only thing left in src/index.ts is the entry-point
 * `import './boot';` so webpack.config.js does not need to change.
 */
