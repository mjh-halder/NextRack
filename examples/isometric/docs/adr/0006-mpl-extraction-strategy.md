...
|---|---|---|
| `src/index.ts` | PARTIAL | All current content (NextRack app shell — theme toggle, view switching, navigation rail, editor-mode modal, switcher menu) moved into new `src/boot.ts`. `index.ts` shrinks to a 3-line `import './boot';` shim. A clean revert to upstream content was rejected because the upstream `index.ts` references demo shape classes (`Computer`, `Database`, `Switch`, `Router`, etc.) that have been deleted and would not compile. `webpack.config.js` left untouched (per project scope rules) so the shim continues to satisfy the existing entry point. `index.ts` therefore stays as a 3-line modified MPL-2.0 file and is published; its content carries no NextRack secrets. |
| `src/shapes/isometric-shape.ts` | PARTIAL | The 8 NextRack-added subclasses were moved to per-family files under `src/shapes/<name>/<name>-shape.ts` (rectangle hosts `PolygonShape` + `RectangleShape` + `ProportionalRectangleShape`; tube hosts `TubeShape` + shared `PHI`/`SQRT2`/`TUBE_ARC_ROTATION`; pipe hosts `PipeShape` + `PIPE_ARC_ROTATION`; circle/duct/channel one each). The base `IsometricShape` class itself was modified in place by NextRack (port plumbing in constructor + `usePorts`/`updatePortPositions`, modifier-key getters `twist`/`scaleTopX/Y`/`shedRoof*`, `addTools` signature + filter, `toggleView` body, new `apply2DHitArea` method). A `NextrackIsometricShape extends IsometricShape` wrapper layer was considered but rejected — it would add an inheritance layer purely for license separation while introducing constructor-ordering hazards. The 4 original demo subclasses (`CuboidShape`, `ProportionalCuboidShape`, `CylinderShape`, `PyramidShape`) were already deleted. `isometric-shape.ts` therefore stays as a modified MPL-2.0 file and must be published. |
